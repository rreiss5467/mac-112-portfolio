...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Desktop/Classes F24/DS 112/mac-112-portfolio/data/imdb_5000_messy.csv")
```
...
imdb_temp <- imdb_temp %>% 
  mutate(color = str_replace(color, "Color", "COLOR"),
         ...
         color = str_replace(color, "Black_White", "B&W"),
         color = str_replace(color, "Black_White", "Black and White"))
```
...
```
# There are no actors (they are all documentaries), so there are no values for actor_1_facebook_likes.  
\
\
\
\
